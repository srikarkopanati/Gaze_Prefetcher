 ...
 *
 * Modification: PatternTable entries carry a 2-bit saturating confidence
 * counter (range 0..3). Updates strengthen confidence on agreement and
 * weaken it on disagreement. Lookups treat zero-confidence entries as
 * misses. Replacement prefers victims with the lowest confidence, with
...
constexpr int CONF_INIT = 1;
constexpr int CONF_MAX = 3;
constexpr int CONF_MIN = 0;
...
struct PatternTableData {
    ...
    PatternTableData(std::vector<int> pattern, uint64_t pc) :
        pattern(pattern), pc(pc), confidence(CONF_INIT) {}
    PatternTableData(std::vector<int> pattern, uint64_t pc, bool con) :
        pattern(pattern), pc(pc), con(con), confidence(CONF_INIT) {}
    PatternTableData(std::vector<int> pattern, uint64_t pc, bool con, int confidence) :
        pattern(pattern), pc(pc), con(con), confidence(confidence) {}
};
...
private:
    ...
    // Auxiliary tracking: for every key currently resident in the underlying
    // table, remember which set it lives in along with a monotonically
    // increasing LRU tick. This lets us pick a victim by lowest confidence
    // and break ties using LRU when inserting a key whose set is full.
    struct KeyMeta {
        uint64_t set_index;
        uint64_t lru_tick;
    };
    std::unordered_map<uint64_t, KeyMeta> resident_keys_;
    uint64_t lru_counter_ = 0;

    void write_data(Entry& entry, custom_util::Table& table, int row);
    uint64_t build_key(uint64_t trigger, uint64_t second, uint64_t pc, uint64_t region_num);
    uint64_t set_index_of(uint64_t key) const;
    void touch_resident(uint64_t key, uint64_t set_index);
    void drop_resident(uint64_t key);
    int count_resident_in_set(uint64_t set_index) const;
    bool select_confidence_victim(uint64_t set_index, uint64_t& victim_key);
...
public:
    ...
    GazeConf(int ft_size, int ft_ways, int at_size, int at_ways, int pt_size, int pt_ways, int pb_size, int pb_ways, int cpu);
    GazeConf();
